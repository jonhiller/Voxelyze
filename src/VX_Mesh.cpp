...
//	offsets.resize(vertexCounter*3);
...
void CVX_MeshRender::updateMesh(viewColor color)
{
	//location
	...
	for (int i=0; i<qCount; i++){
		...
		switch (color){
			...
			case KINETIC_ENERGY:
				...
				break;
			case STRAIN_ENERGY: case ENG_STRAIN: case ENG_STRESS:
				jetValue = linkMaxColorValue(vx->voxel(quadVoxIndices[i]), color) / maxVal;
				break;
			case DISPLACEMENT:
...
float CVX_MeshRender::linkMaxColorValue(CVX_Voxel* pV, viewColor color)
{
	...
	for (int i=0; i<6; i++){
		...
		if (pL){
			switch (color){
				case STRAIN_ENERGY: thisVal = pL->strainEnergy(); break;
				case ENG_STRESS: thisVal = pL->axialStress(); break;
				case ENG_STRAIN: thisVal = pL->axialStrain(); break;
			}
		}
