...
public:
	...
	CVX_External(const CVX_External& eIn) {_extRotationQ = 0; mfcs = 0; *this = eIn;} //!< Copy constructor
	...
	bool isEmpty() {return (dofFixed == 0 && extForce==Vec3D<float>() && extMoment==Vec3D<float>() && !mfcs);} //!< returns true if this external is empty - i.e is exerting no effect on a voxel
	...
	void addMfc(double cX = 0, double cY = 0, double cZ = 0, double cRX = 0, double cRY = 0, double cRZ = 0); //!< Adds a multi-freedom constraint to this voxel (Advanced). Currently only observed in the linear solver. Assumed form cX*Ux + cY*uY + ... + cRZ*uRY = 0, where u_ is the displacement of the indicated degree of freedom. @param[in] cX X displacement coefficient.  @param[in] cY Y displacement coefficient.  @param[in] cZ Z displacement coefficient. @param[in] cRX X rotation coefficient. @param[in] cRY Y rotation coefficient. @param[in] cRZ Z rotation coefficient.
	int mfcCount() const; //!< Returns the number of multi-freedom constraints on this voxel
	void removeMfc(int mfcIndex); //!< Removes the specified mfcIndex. Frees memory if none left
	void clearAllMfcs(); //!< Clears all the multi-freedom constraint from this voxel (and frees memory).
	//bool hasMfc() { return mfc; } //!< Returns true if this external contains a multifuction constraint.
	double* mfcElements(int mfcIndex) const; //!< returns a pointer to the first element (X coefficient) of the mfc array. Length is guaranteed to be 6 - the pointer, if non-null, can safely be used to access all 6 elements. 
...
private:
	...
	std::vector<std::vector<double>>* mfcs; //pointer to save allocation space for the vast majority of voxels that will not have this
